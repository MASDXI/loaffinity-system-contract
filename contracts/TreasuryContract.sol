...
contract TreasuryContract is ITreasury, Proposal, Initializer, NativeTransfer {
    ...
    modifier onlyProposer() {
        require(
            _commiteeContract.isProposer(msg.sender),
            "treasury: onlyProposer can call"
        );
        _;
    }
    ...
    modifier onlyCommittee() {
        require(
            _commiteeContract.isCommittee(msg.sender),
            "treasury: onlyCommittee can call"
        );
        _;
    }
    ...
    modifier onlyAgent() {
        require(
            _commiteeContract.isAgent(msg.sender),
            "treasury: onlyAgent can call"
        );
        _;
    }
    ...
    function _getProposal(
        bytes32 proposalId
    ) private view returns (ProposalSupplyInfo memory) {
        ProposalSupplyInfo memory data = _supplyProposals[proposalId];
        require(data.blockNumber != 0, "treasury: proposal not exist");
        return data;
    }

    function getProposalSupplyInfoByProposalId(
        bytes32 proposalId
    ) public view returns (ProposalSupplyInfo memory) {
        return _getProposal(proposalId);
    }

    function getProposalSupplyInfoByBlockNumber(
        uint256 blockNumber
    ) public view returns (ProposalSupplyInfo memory) {
    ...
    function propose(
        ...
        if (proposeType == ProposalType.RELEASED) {
            require(
                account != address(0),
                "treasury: propose released to zero address"
            );
        } else {
            require(
                account == address(0),
                "treasury: propose locked to non-zero address"
            );
        }
        require(
            blockProposal[blockNumber] == bytes32(0),
            "treasury: blocknumber has propose"
        );

        bytes32 proposalId = keccak256(
            abi.encode(msg.sender, account, amount, blockNumber)
        );
        _proposal(
            proposalId,
            uint16(_commiteeContract.getCommitteeCount()),
            blockNumber
        );
